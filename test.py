...
class Player:
    def __init__(self, w, h):
        self.rect = pygame.Rect((window.get_width() - w) / 2, (window.get_height() - h) / 2, w, h)
    ...
    def check_collisions(self):
        ...
        if self.rect.top < 0:                 # top border of the window
            self.rect.top = 0
        if self.rect.bottom > window.get_height():         # bottom border of the window
            self.rect.bottom = window.get_height()
        if self.rect[0] < 0:                  # left border of the window
            self.rect[0] = 0;
        if window.get_width() < self.rect[0]+self.rect[2]: # left border of the window
            self.rect[0] = window.get_width() - self.rect[2]
...
walls = Walls()
walls.add(0, 0, 100, window.get_height())
walls.add(window.get_width() - 100, 0, 100, window.get_height())
player = Player(40, 80)
